...
app.post('/api/crm-documents/list', async (req, res) => {
  try {
    ...
    if (!crmConfig?.apiKey || !crmConfig?.locationId || !contactId) {
      return res.status(400).json({ message: 'crmConfig (apiKey, locationId) and contactId are required' });
    }
...
app.post('/api/crm-upload-custom-file', async (req, res) => {
  try {
    ...
    if (!crmConfig?.apiKey || !crmConfig?.locationId || !contactId || !fileData) {
      return res.status(400).json({ message: 'crmConfig (apiKey, locationId), contactId, and fileData (base64 or URL) are required' });
    }
...
app.post('/api/crm-contact-add-note', async (req, res) => {
  try {
    ...
    if (!crmConfig?.apiKey || !crmConfig?.locationId || !contactId || !note) {
      return res.status(400).json({ message: 'crmConfig (apiKey, locationId), contactId, and note are required' });
    }
...
app.post('/api/crm-contact-debug', async (req, res) => {
  try {
    const { crmConfig, contactId } = req.body;
    if (!crmConfig?.apiKey || !crmConfig?.locationId || !contactId) {
      return res.status(400).json({ message: 'crmConfig (apiKey, locationId) and contactId required' });
    }
...
cron.schedule(CRON_SCHEDULE, async () => {
  ...
  if (!crmConfig.apiKey || !crmConfig.locationId) {
    console.warn('Cron sync skipped: CRM env vars not set (CRM_API_KEY, CRM_LOCATION_ID)');
    return;
  }
...
app.post('/api/contact-lens-reminders/process', async (req, res) => {
  try {
    const { crmConfig, supabaseConfig } = req.body;
    if (!crmConfig?.apiKey || !crmConfig?.locationId) {
      return res.status(400).json({ message: 'crmConfig (apiKey, locationId) required' });
    }
